...
<body>
    ...
    {{-- Jika jenis = penjualan --}}
    @if ($jenis == 'penjualan')
        @foreach ($data as $penjualan)
            <table>
                <tr>
                    <th style="text-align: left">ID Penjualan</th>
                    <td>{{ isset($penjualan->id) ? 'PJL' . str_pad($penjualan->id, 5, '0', STR_PAD_LEFT) : '-' }}</td>
                </tr>
                <tr>
                ...
                {{--  <tr>
                    <th style="text-align: left">Tanggal Terima</th>
                    <td>{{ \Carbon\Carbon::parse($penjualan->tanggal_terima)->format('d-m-Y') }}</td>
                </tr>  --}}
                {{--  <tr>
            ...
            <h5 style="margin-top: 10px;">Detail Obat Terjual</h5>
            {{--  <table>
                <thead>
                    <tr>
                        <th>NO</th>
                ...
                <tbody>
                    @foreach ($penjualan->detailPenjualans as $i => $detail)
                        <tr>
                            <td>{{ $i + 1 }}</td>
            ...
            <table>
                ...
                <tbody>
                    ...
                    @foreach ($penjualan->detailPenjualans as $i => $detail)
                        @php
                            $subtotal = $detail->jumlah_beli * $detail->obat->harga;
                            $total += $subtotal;
                        @endphp
                        <tr>
                            <td>{{ $i + 1 }}</td>
